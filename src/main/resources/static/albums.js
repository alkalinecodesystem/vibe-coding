    ...
    playAlbumButtons.forEach(button => {
        ...
        button.addEventListener('click', async function (event) {
            ...
            try {
                ...
                if (!data) {
                    alert('No album data received from the server.');
                    const modal = bootstrap.Modal.getInstance(document.getElementById('albumModal'));
                    if (modal) modal.hide();
                    return;
                }
                ...
                // Pause any current playback when loading new album
                ...
                if (isRepeat) {
                    ...
                    currentAudioPlayer.play().catch(e => {
                        if (e.name !== 'AbortError') {
                            console.error('Play failed on repeat:', e);
                        }
                    });
                } else {
                    // Stop playing when song ends
                    isPlaying = false;
